...
CHMORT5 <- function (Data.Name,PeriodEnd=NULL)
{
  ...
  for (i in seq_along (ageseg)) {
    ...
    segdata$exposure[segdata$b3 >= (segdata$tl - ageseg[[i]][2]) & segdata$b3 < (segdata$tl - ageseg[[i]][1]) ] <- 0.5
    segdata$exposure[segdata$b3 >= (segdata$tl - ageseg[[i]][1]) & segdata$b3 < (segdata$tu - ageseg[[i]][2]) ] <- 1
    segdata$exposure[segdata$b3 >= (segdata$tu - ageseg[[i]][2]) & segdata$b3 < (segdata$tu - ageseg[[i]][1]) ] <- 0.5
    
    segdata$death[segdata$b3 >= (segdata$tl - ageseg[[i]][2]) & segdata$b3 < (segdata$tl - ageseg[[i]][1]) & (segdata$b7 >= ageseg[[i]][1] & segdata$b7 < ageseg[[i]][2])] <- 0.5
    segdata$death[segdata$b3 >= (segdata$tl - ageseg[[i]][1]) & segdata$b3 < (segdata$tu - ageseg[[i]][2]) & (segdata$b7 >= ageseg[[i]][1] & segdata$b7 < ageseg[[i]][2])] <- 1
    segdata$death[segdata$b3 >= (segdata$tu - ageseg[[i]][2]) & segdata$b3 < (segdata$tu - ageseg[[i]][1]) & (segdata$b7 >= ageseg[[i]][1] & segdata$b7 < ageseg[[i]][2])] <- ifelse(is.null(PeriodEnd),1,0.5)
  ...
  mort_rates   <- list(c(data.frame(dpro=deathprob[1:1])),
...
for (i in seq_along (BirthNg)) {
  ...
  segdata$exposure[segdata$b3 >= (segdata$tl - BirthNg[[i]][2]) & segdata$b3 < (segdata$tu - BirthNg[[i]][1]) ] <- 1
