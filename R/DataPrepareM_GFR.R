# DataPrepareM_GFR function: prepare Birth and Exposure data to calculate AAGFR for MMR calculation
# Mahmoud Elkasabi
# 10/05/2020
...
DataPrepareM_GFR <- function(Dat, PeriodEnd = NULL, Period = NULL)
{
  ...
  newdata <- c("ID", "v021", "v005", "v008", "v011", "v022", "rweight")
  def2 <- Dat[newdata]
  ...
  BirthEx <- merge(stats::aggregate(list(def4$birth, def4$exposure, def4$exposureg),
                                    list(def4$ID, def4$v021, def4$v022, def4$age5), sum),
                   stats::aggregate(def4$rweight, list(def4$ID), mean), by = "Group.1")

  names(BirthEx) <- c("ID", "v021", "v022", "age5", "birth", "exposure",
                      "exposureg", "rweight")
