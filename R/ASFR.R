...
ASFR <- function (Data.Name, EverMW=NULL,AWFact=NULL,PeriodEnd=NULL,Period=NULL,Class=NULL)
{
  ...
  if (is.null(Class)){
    ...
    SE    = (survey::svyby(~birth, by=~age5, denominator=~exposure, design=dstrat, survey::svyratio))[,3]
    ...
    DEFT  = sqrt(survey::svyby(~birth, by=~age5, denominator=~exposure, design=dstrat,deff="replace", survey::svyratio)$DEff)
    RSE   = survey::cv(survey::svyby(~birth, by=~age5, denominator=~exposure, design=dstrat, survey::svyratio))
    LCI   = ASFR - (2*SE)
    UCI   = ASFR + (2*SE)

    RESULTS <- cbind.data.frame(AGE,round(ASFR,3),round(SE,3),round(N,0),round(WN,0),round(DEFT,3),round(RSE,3),round(LCI,3),round(UCI,3))
  ...
  #######For Class Indicators; #################################################################################################
    ...
    dimnames(RESULTS) <- list(NULL, c("Class","AGE","ASFR","SE", "N", "WN", "DEFT", "RSE", "LCI", "UCI") )
    ...
    for (j in 1:(max(Dat$DomID))) {
      ...
      RESULTS0 <- cbind.data.frame(attributes(Dat[[Class]])$levels[[j]],AGE,round(ASFR,3),round(SE,3),round(N,0),round(WN,0),round(DEFT,3),round(RSE,3),round(LCI,3),round(UCI,3))
