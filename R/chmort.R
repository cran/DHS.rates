...
#'
#' @export
chmort <- function (Data.Name, JK=NULL, Strata=NULL, Cluster=NULL, Weight=NULL, PeriodEnd=NULL, Period=NULL, Class=NULL)
{
  ...
  if (!("b3" %in% names(Data.Name))) stop({message("Error: b3 is missing")})
  if (!("b7" %in% names(Data.Name))) stop({message("Error: b7 is missing")})

 Data.ready <- as.data.frame(Data.Name[!Data.Name$v005==0,])

  if (is.null(Period)){Data.ready$period = 60} else {Data.ready$period = Period * 12}   # refence period in years
  if (is.null(PeriodEnd)){Data.ready$periodend = 0} else {Data.ready$periodend = PeriodEnd * 12}  # End of refence period in years; 0 when the end is the DOI
  ...
  chmortdat$id <- c(as.factor(chmortdat$v021))
    ...
    if (is.null(JK)){
      ...
      dimnames(RESULTcmr) <- list(c("NNMR","PNNMR","IMR","CMR","U5MR"),c("Rate","SE", "N", "RSE","LCI","UCI","iterations"))
      for (i in 1:(nrow(RESULTcmr))) {
        RESULTcmr[i,1]= CHMORT[i]
  ...
  #######For Class Indicators; #################################################################################################
    ...
    chmortdat<- Data.ready[, c("v021", "v022", "rweight", "v008", "b3", "b7", "tu", "tl","periodend",Class)]
    ...
    for (j in 1:(max(chmortdat$DomID))) {
      ...
      NBIRTH = NBIRTHS(DatD)[[1]]
        ...
        if (is.null(JK)){
            ...
            RESULTS0 <- cbind.data.frame(attributes(DatD[[Class]])$levels[[j]],round(CHMORT,2),NBIRTH)
          ...
          RESULTS0 <- cbind.data.frame(attributes(DatD[[Class]])$levels[[j]],round(CHMORT,2),round(RateSE,2), NBIRTH, round(RateSE/CHMORT,2), round(CHMORT - 2*RateSE,2), round(CHMORT + 2*RateSE,2), PSU)
