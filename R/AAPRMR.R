                   ...
                   PeriodEnd = NULL, Period = NULL)
{
  ...
  if (is.null(PeriodEnd)){
    cat("\n", crayon::white$bgBlue$bold("The current function calculated AAPRMR based on a reference period of"),
  ...
  if (is.null(JK)){
    ...
    for (i in unique(as.numeric(DeathEx$id)))
    {
      ...
      JKres[i,1] <- sum(ASPRMRj[1:7] * AGEDISTj)
    }
    AAPRMRjf = JKres[1:PSU, 1]
    JKSEf = ((PSU * aaprmr[1] - (PSU-1) * AAPRMRjf)-aaprmr[1])^2
